...
export const TIER_META: Record<Tier, { label: string; color: string; description: string }> = {
  ...
  practiced: { label: "Practiced", color: "#8b5cf6", description: "You've answered at least two items in this method and reviewed at least one (reps ≥ 2)." },
  proficient:{ label: "Proficient",color: "#22c55e", description: "≥30% of the bank reviewed (reps ≥ 2 each) and at least one item on a long-interval mastery schedule." },
  mastered:  { label: "Mastered",  color: "#f59e0b", description: "≥50% of the bank held at long-interval recall (≥4 reps, ≥21-day interval) — the conservative bar for an external statement." },
};

// Compute the tier from an SRS rollup. Walks the ladder top-down; first
// matching tier wins.
//
// Thresholds tuned for early-user motivation while keeping "Mastered" a
// public-facing claim that means something:
//   • Familiar  — at least one attempt (reps ≥ 0 in SRS but logged)
//   • Practiced — at least 2 attempts AND at least 1 reviewed
//                 (≥2 reps on at least one item in the method)
//   • Proficient — at least 30% of the bank reviewed AND ≥1 mastered
//                 (gives Pro a meaningful step beyond Practiced)
//   • Mastered  — at least 50% of the bank held at long-interval recall
//                 (still conservative enough for a Statement of
//                  Competency, but reachable in weeks not months)
//
// `max(1, …)` keeps single-question methods reachable.
export function tierFromMastery(s: MasterySummary): Tier {
  const total = s.total | 0;
  if (total === 0) return "untouched";
  const reqMastered  = Math.max(1, Math.ceil(total * 0.5));
  const reqReviewed  = Math.max(1, Math.ceil(total * 0.3));
  if (s.mastered >= reqMastered) return "mastered";
  if (s.reviewed >= reqReviewed && s.mastered >= 1) return "proficient";
  if (s.attempted >= 2 && s.reviewed >= 1) return "practiced";
